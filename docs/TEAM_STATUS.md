...
Last refreshed: 2026-08-29 (`main` @ `278344b`). Scope: Fayda + USSD still out of
MVP — see [`.cursor/rules/decisions.mdc`](../.cursor/rules/decisions.mdc) D1.

**On `main`:** Tasks 1–8, 10, 11, 12, 13, 14, 16, 17, 18, 19, 23, 24, 25, 26, 30
+ docs (Fayda/USSD drop, test-case matrix, MoMo OpenAPI).

**Not on `main`:** Task 22 (in review — `task22-obsan-rbac-audit`), Tasks 20, 21,
33 (open branches), Task 18 follow-up commits (7 on branch).

**`main` does not compile** until Task 22 merges: PR #33 registered
`DocumentsReportingController` without the file (it lives on `task20-biruk-reporting`).
Task 22 removes that registration; Biruk restores it with Task 20.
...
|---|---|---|---|---|---|
| **Obsan** | Platform | Task 22 in review → Task 15 | `task22-obsan-rbac-audit` | **in review** — open PR | Task 3 ✅, Task 14 ✅ |
...
| **Abenezer** | Loans & Credit | Fix D-30-01, D-30-02; Task 31 defect list | `task18-abenezer-loan-ui` | 7 commits unmerged on branch | Task 22 merge for `@Roles` on loans |
| **Biruk** | Admin & Reporting | Merge Task 20 + 21 (remove mock data first) | `task20-biruk-reporting`, `task21-biruk-tenant-admin-dashboard` | **in progress** — branches open | Task 22 merge unblocks clean `main` build path |
| **Liya** | Member Self-Service | Task 34 doc template; verify member routes after Task 22 | — | Tasks 7, 23, 25, 26 **merged** | Task 22 for enforced RBAC |

**Active gates:**
1. **Merge Task 22** — unbreaks `main`, enforces RBAC (D-30-05), ships audit log (D-30-06).
2. **Biruk merge Task 20** — restores reporting controller + real ledger reads (D-30-03, D-30-04).
3. **Biruk merge Task 21** — real tenant-admin dashboard (placeholder on `main` today).
4. **Melkamu open PR for Task 33** — backup/DR rehearsal + RLS check (covers part of Task 28).
...
- **Task 30 UAT matrix** on `main`: 19 PASS, 6 FAIL (D-30-01 through D-30-06). Technical
  dry-run only — staff UAT session still pending (Melkamu + Biruk).

---

## What Task 22 changes for everyone (read after it merges)
...
3. **Re-verify your vertical** after merge — Obsan applied `@Roles` to endpoints that
   landed while Task 22 was open (tenants, member self-service, loan list, member delete).
...
|---|---|---|---|
| 1–3, 5 Backend + auth | various | **merged** | |
| 4 Login & role routing | `task4-obsan-login-routing` | **merged** (PR #22) | |
| 13 Ledger engine | `task13-obsan-ledger` | **merged** (PR #23) | |
| 22 Security & Audit / RBAC | `task22-obsan-rbac-audit` | **in review** | RolesGuard, audit log, `rbac-matrix.md`; also fixes `main` compile break |
| 15 Offline-sync | `task15-obsan-offline-sync` | **ready** | Task 14 merged — can start after Task 22 |
| 28 RLS concurrent load | — | partial | Melkamu's Task 33 branch has isolation check; concurrent load still Obsan |
...
| 31 Bug triage | — | **ready** | owns D-30-01 through D-30-06 in `test-case-matrix.md` |
...
| 20 Documents & Reporting | `task20-biruk-reporting` | **in progress** | controller + real ledger reads; **not merged** — fixes D-30-03, D-30-04 |
...
|---|---|
| **Task 22 (in review)** | clean `main` build; D-30-05, D-30-06 closed |
| Task 20 | reporting endpoints; D-30-03, D-30-04 |
| Task 21 | real tenant-admin dashboard |
| Task 33 | Task 28 partial coverage |
| Task 22 merged | everyone re-tests their vertical under enforced RBAC |
...
As of 2026-08-29 (`main` @ `278344b`):

- `origin/main` — through Task 24, 25, 26, 30; **does not compile** (missing reporting controller file)
- `origin/task22-obsan-rbac-audit` — Task 22 + build fix (**open PR**)
- `origin/task20-biruk-reporting` — Task 20 (2 commits ahead)
- `origin/task21-biruk-tenant-admin-dashboard` — Task 21 (11 commits ahead)
- `origin/task33-melkamu-backup-restore` — Task 33 (11 commits, current with `main`)
...
- Stale after merge: `task1`…`task16`, older docs branches — delete when convenient
