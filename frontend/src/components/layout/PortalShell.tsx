...
import { Menu, Search, Bell, X, HelpCircle, Settings, LogOut, User, ShieldAlert, CheckCircle2, Info } from 'lucide-react';
...
export interface PortalShellProps {
  ...
  className?: string;
  /** @deprecated Theme is owned by ThemeProvider (`isms-theme`). Ignored. */
  darkMode?: boolean;
  /** Optional extra callback after `useTheme().toggleTheme`. */
...
export const PortalShell: React.FC<PortalShellProps> = ({
  ...
  const renderSidebar = () => (
    <div className="flex flex-col h-full bg-white text-slate-700 dark:bg-midnight dark:text-slate-300">
      <div className="p-5 flex items-center justify-between border-b border-slate-200 dark:border-white/5">
        <Link href={homeHref} className="flex items-center gap-2.5 min-w-0 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/60">
          ...
          <div className="flex flex-col leading-none min-w-0">
            <span className="notranslate font-display text-[13px] font-bold text-midnight dark:text-white tracking-[0.2em] uppercase">ISMS</span>
            <span className="text-[8px] text-slate-400 dark:text-white/30 tracking-[0.15em] uppercase font-sans mt-0.5">{t('brandTagline')}</span>
          </div>
        </Link>
        <button
          type="button"
          onClick={() => setMobileSidebarOpen(false)}
          className="md:hidden p-1.5 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-white/10 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/60"
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 flex flex-col gap-5" aria-label={`${t(portalName)} navigation`}>
        {sections.map((section, si) => (
          <div key={si}>
            {section.label && (
              <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 px-3 mb-2">{t(section.label)}</p>
            )}
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      active
                        ? 'bg-gold/10 text-gold-dark shadow-sm dark:bg-white/10 dark:text-gold'
                        : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-white/5 dark:hover:text-white',
                    )}
                  >
                    {item.icon && (
                      <span className={cn('shrink-0 transition-colors', active ? 'text-gold-dark dark:text-gold' : 'text-slate-400 group-hover:text-slate-600 dark:text-slate-500 dark:group-hover:text-slate-300')}>
                        {item.icon}
                      </span>
                    )}
      ...
      {user && (
        <button
          type="button"
          onClick={goProfile}
          className="mx-3 mb-4 p-3 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-2.5 text-left hover:bg-slate-100 dark:bg-white/5 dark:border-white/10 dark:hover:bg-white/10 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/60"
        >
          ...
          <div className="min-w-0">
            <div className="text-[12px] font-semibold text-slate-900 dark:text-white truncate">{user.name}</div>
            <div className="text-[10px] text-gold-dark dark:text-gold/70 truncate">{user.role}</div>
          </div>
        </button>
      )}
    </div>
  ...
  return (
    <div className={cn('min-h-screen flex bg-surface text-slate-900 dark:text-slate-100', className)}>
      ...
      <aside id="dashboard-sidebar" className="w-60 bg-white dark:bg-midnight fixed top-0 left-0 h-screen z-40 hidden md:flex flex-col shrink-0 border-r border-slate-200 dark:border-white/5">
        {renderSidebar()}
      </aside>

      {mobileSidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex" role="dialog" aria-modal="true" aria-label={t('shell.openMenu')}>
          <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm" onClick={() => setMobileSidebarOpen(false)} />
          <div className="relative w-60 bg-white dark:bg-midnight h-full shadow-2xl z-10">{renderSidebar()}</div>
        </div>
      )}

      <div className="flex-1 flex flex-col min-h-screen min-w-0 md:ml-60">
        <header className="isms-gt-header-pad h-16 bg-white/95 backdrop-blur-sm dark:bg-midnight px-4 md:px-6 flex items-center justify-between sticky top-0 z-30 border-b border-slate-200 dark:border-white/5">
          <div className="flex items-center gap-3 min-w-0">
            <button
              type="button"
              onClick={() => setMobileSidebarOpen(true)}
              className="md:hidden p-2 text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-white/10 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/60"
            ...
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              {currentPage && (
                <>
                  <span className="text-slate-300 dark:text-white/20 hidden sm:inline" aria-hidden="true">/</span>
                  <span className="text-sm text-slate-700 dark:text-white/80 font-medium truncate hidden sm:inline">{t(currentPage.label)}</span>
                </>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-1">
            {onOpenSearch && (
              <button
                ...
                className="p-2 text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-white/10 rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/60"
            ...
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="p-2 text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-white/10 rounded-lg transition-colors relative focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/60"
                aria-label={unreadCount > 0 ? t('shell.notificationsUnread', { count: unreadCount }) : t('shell.notifications')}
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-elevated z-50 overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
                    <h3 className="font-bold text-sm text-slate-800 dark:text-slate-100">{t('shell.notifications')}</h3>
                  ...
                  <div className="max-h-72 overflow-y-auto">
                    {notifications.length === 0 ? (
                      <div className="px-4 py-10 text-center">
                        ...
                        <p className="text-sm font-medium text-slate-600 dark:text-slate-300">{t('shell.allCaughtUp')}</p>
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{t('shell.alertsHint')}</p>
                      </div>
                    ) : (
                      ...
                      <div className="py-1">
                        {canGoProfile && (
                          <button
                            ...
                            onClick={goProfile}
                          >
                            <User className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500" /> {t('shell.viewProfile')}
                          </button>
                        )}
                        {canGoSettings && (
                          <button
                            ...
                            onClick={goSettings}
                          >
                            <Settings className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500" /> {t('shell.settings')}
                          </button>
                        )}
                      </div>
                    )}
        ...
        <main id="main-content" className="flex-1 overflow-y-auto bg-surface dark:bg-surface flex flex-col">
