...
function LogoMark() {
  ...
  return (
    <div className="flex items-center gap-3">
      <div
        className="notranslate w-11 h-11 rounded-full bg-gold text-midnight flex items-center justify-center font-display text-[10px] font-bold tracking-wider shadow-[0_0_0_3px_rgba(197,155,39,0.2)]"
        aria-hidden="true"
      >
        ISMS
      </div>
      <div className="leading-none">
        <p className="notranslate font-display text-sm font-bold text-slate-900 dark:text-white tracking-[0.2em] uppercase">
          ISMS
        </p>
...
export default function WelcomeNav({
  ...
  const { t } = useLang();
  return (
    <div className="isms-gt-header-pad flex items-center justify-between gap-4 py-5">
      <LogoMark />
      <div className="flex items-center gap-2 sm:gap-3">
        {user && (
          <p className="hidden lg:block text-[11px] text-slate-500 dark:text-white/50 truncate max-w-[12rem]">
            {user.fullName}
            <span className="text-gold-dark dark:text-gold/80"> · {t(`role.${user.role}`)}</span>
          </p>
        )}
